...
const Songs = (props) => {
    ...
    const toggleSong = (selected, song) => {
        if(selected) {
            ...
            props.setSongs(props.toggledSongs.filter(songid => songid !== song.id ? songid : null));
        }
    }

    return (
        <div className={css(ss.songWrapper)}>
            <Sort checkmark={checkmark} checkedCategories={props.checkedCategories} handleSetCheckedCategories={props.setCheckedCategories} handleSelectAll={toggleCheckmark} />
            {
                props.songs.length ? props.songs.map(song => 
                    <Song 
                        key={song.id} 
                        details={song} 
                        handleToggle={() => toggleSong(!props.toggledSongs.includes(song.id), song)}
