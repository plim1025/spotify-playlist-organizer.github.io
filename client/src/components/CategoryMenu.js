...
const CategoryMenu = (props) => {
    ...
    const setCategories = category => {
        if(props.checkedCategories.indexOf(category) > -1) {
            const categories = props.checkedCategories.filter(item => item !== category);
            props.handleSetCheckedCategories(categories);
        } else {
            props.handleSetCheckedCategories([...props.checkedCategories, category]);
        }
    }

    return (
        <Modal className={css(ss.wrapper)} open={props.visible} onClose={props.handleClose}>
                <List className={css(ss.list)}>
                    <div className={css(ss.listTitle)}>Select Categories to Display</div>
                    {['Album', 'Artists', 'Danceability', 'Energy', 'Instrumentalness', 'Duration', 'Loudness', 'Name', 'Popularity', 'Valence', 'Speechiness', 'Tempo', 'Year']
                        .map(category => 
                            <ListItem key={category} onClick={() => setCategories(category)} className={css(ss.listItem)}>
                                ...
                                <ListItemText primary={category === 'Duration' ? 'Length' : category === 'Instrumentalness' ? 'Instrumental' : category === 'Speechiness' ? 'Speech' : category === 'Valence' ? 'Positivity' : category} />
                            </ListItem>
                        )
                    }
                </List>
