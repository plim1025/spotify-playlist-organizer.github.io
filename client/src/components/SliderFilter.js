...
const SliderFilter = props => {
    ...
    return initialRange.length === 2 && range.length === 2 ? (
        <div className={css(ss.wrapper)}>
            ...
            <Slider
                ...
                max={initialRange[1]}
                step={
                    props.category === 'duration' ||
                    props.category === 'loudness' ||
                    props.category === 'popularity' ||
                    props.category === 'tempo'
                        ? 1
                        : 0.01
                }
                value={range}
                ...
                valueLabelFormat={
                    props.category === 'duration'
                        ? ms => msToMinsSecs(ms)
                        : props.category === 'danceability'
                        ? decimal => decimalToPercentage(decimal)
                        : props.category === 'energy'
                        ? decimal => decimalToPercentage(decimal)
                        : props.category === 'instrumental'
                        ? decimal => decimalToPercentage(decimal)
                        : props.category === 'popularity'
                        ? number => number + '%'
                        : props.category === 'speechiness'
                        ? decimal => decimalToPercentage(decimal)
                        : props.category === 'valence'
                        ? decimal => decimalToPercentage(decimal)
                        : label => label
                }
            />
